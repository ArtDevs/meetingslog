...
CREATE TABLE IF NOT EXISTS ml_user_group_ref (
  id INTEGER PRIMARY KEY AUTO_INCREMENT,
  user_id INTEGER NOT NULL REFERENCES ml_users(id),
  group_id INTEGER NOT NULL REFERENCES ml_groups(id),
  CONSTRAINT UNIQUE KEY ml_user_role_ref_key(user_id,group_id)
);
